      ...
      <div class="row">
        <div class="span12 columns">
          <div class="well">
            ...
            $options_brand = array(0 => "all");
            foreach ($brand as $row)
            {
              $options_brand[$row['id']] = $row['brandName'];
            }
            //save the columns names in a array that we will use as filter         
            $options_brand_sort = array();    
			...
			$options_brand_sort['brandName'] = 'brandName';
			$options_brand_sort['id'] = 'id';
...
height: 26px;"');
              ...
              echo form_dropdown('order', $options_brand_sort, $order, 'class="span2"');
              ...
              echo form_button('myreset',"Reset",'class="btn btn-danger" onclick=window.location.href="'.base_url('admin/brand').'"');
          ...
          <table class="table table-striped table-bordered table-condensed">
            <thead>
              <tr>
                <th class="header">#</th>
                <th class="yellow header headerSortDown">Brand Name</th>
                <th class="yellow header headerSortDown">Parent Brand</th>
            ...
            <tbody>
              ...
              foreach($brand as $row)
              {
				...
				if($row['status']==1){
                	echo '<td><a href="'.site_url("admin").'/brand/updatestatus/'.$row['id'].'/0/">Active</a></td>';
				}else{
                	echo '<td><a href="'.site_url("admin").'/brand/updatestatus/'.$row['id'].'/1/">De-active</a></td>';
				}
                echo '<td class="crud-actions">
                  <a href="'.site_url().'admin/brand/update/'.$row['id'].'" class="btn btn-info">view & edit</a>  ';
				  $login_user_details = $this->session->userdata('user_details');
				 if($login_user_details[0]['admin']){
                 	//echo '<a href="'.site_url().'admin/brand/delete/'.$row['id'].'" class="btn btn-danger">delete</a>';
				 }
                echo '</td>';
                echo '</tr>';
              }
